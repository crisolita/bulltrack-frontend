...
export function BullItem({ bull, rank, onToggleFavorite }: BullItemProps) {
  ...
  const getBadgeColor = (type: "origen" | "uso") => {
    if (type === "origen") {
  ...
  const origenLabel = bull.origen === "propio" ? "Propio" : bull.origen === "catalogo" ? "Catálogo" : "Favorito";
  const usoLabel = bull.uso === "vaquillona" ? "Para vaquillona" : "Para vaca";

  const bullImage = "https://images.unsplash.com/photo-1560493676-04071c5f467b?w=400&h=300&fit=crop&auto=format";
          ...
          <div className="flex gap-[24px] items-center p-[8px]">
            ...
            <div className="flex flex-col gap-[16px] items-start">
              ...
              <div className="flex gap-[8px] items-start">
                <div className={`${origenColors.bg} h-[24px] rounded-[8px] border-[1.5px] ${origenColors.border}`}>
                  <div className="flex h-full items-center justify-center px-[8px] py-[6px] rounded-[inherit]">
                    <div className="flex items-center justify-center px-[8px]">
                      <span className={`font-semibold text-[10px] leading-[12px] ${origenColors.text}`}>
                        {origenLabel}
                      </span>
                    </div>
                  </div>
                </div>
                <div className={`${usoColors.bg} h-[24px] rounded-[8px] border-[1.5px] ${usoColors.border}`}>
                  <div className="flex h-full items-center justify-center px-[8px] py-[6px] rounded-[inherit]">
                    <div className="flex items-center justify-center px-[8px]">
                      <span className={`font-semibold text-[10px] leading-[12px] ${usoColors.text}`}>
                        {usoLabel}
                      </span>
                    </div>
                  </div>
                ...
                <svg className="block size-full" fill="none" viewBox="0 0 88 87.6">
                  <g>
                    <g>
                      ...
                      <path d="M44.4 11.6L44.4 44" stroke="#D9D9D9" strokeWidth="0.8" />
                      <path d="M44.4 44L72.4 60.8" stroke="#D9D9D9" strokeWidth="0.8" />
                      <path d="M44.4 44L16.4 60.8" stroke="#D9D9D9" strokeWidth="0.8" />
                    </g>
                    <path d="M44.4 20L54 40L44.4 35L34.8 40Z" fill="#CCF0DA" fillOpacity="0.5" stroke="#3ED97C" strokeWidth="2" />
                  </g>
                </svg>
              </div>
            </div>
              ...
              >
                <svg className="size-[24px]" fill="none" viewBox="0 0 24 24">
                  <path 
                    ...
                    stroke={bull.favorites && bull.favorites.length > 0 ? "#1c2620" : "white"} 
                    strokeLinejoin="round" 
                    strokeWidth="1.5"
                    fill={bull.favorites && bull.favorites.length > 0 ? "#1c2620" : "none"}
                  />
                </svg>
              </button>
            </div>
